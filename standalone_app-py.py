...
# Define analyst options (hardcoded to avoid importing)
ANALYST_ORDER = [
...
# Define model options (hardcoded to avoid importing)
LLM_ORDER = [
    ("[anthropic] claude-3.5-haiku", "claude-3-5-haiku-latest", "Anthropic"),
    ("[anthropic] claude-3.5-sonnet", "claude-3-5-sonnet-latest", "Anthropic"),
    ("[anthropic] claude-3.7-sonnet", "claude-3-7-sonnet-latest", "Anthropic"),
    ("[deepseek] deepseek-reasoner", "deepseek-reasoner", "DeepSeek"),
    ("[deepseek] deepseek-v3", "deepseek-chat", "DeepSeek"),
    ("[gemini] gemini-2.0-flash", "gemini-2.0-flash", "Gemini"),
    ("[gemini] gemini-2.0-pro", "gemini-2.0-pro-exp-02-05", "Gemini"),
    ("[groq] llama-3.3 70b", "llama-3.3-70b-versatile", "Groq"),
    ("[openai] gpt-4.5", "gpt-4.5-preview", "OpenAI"),
    ("[openai] gpt-4o", "gpt-4o", "OpenAI"),
    ("[openai] o1", "o1", "OpenAI"),
    ("[openai] o3-mini", "o3-mini", "OpenAI"),
]

# Create a simplified version of the run_hedge_fund function
def simple_run_hedge_fund(
    tickers, 
    start_date, 
    end_date, 
    portfolio, 
    ...
    """
    A simplified version of run_hedge_fund that returns mock data for demonstration purposes.
    In a real application, this would make actual API calls and use the real agents.
    """
    # Create mock data for demonstration
    decisions = {}
    analyst_signals = {}
    
    # Generate mock decisions for each ticker
    for ticker in tickers:
        ...
        decisions[ticker] = {
            ...
            "reasoning": f"Mock reasoning for {action.upper()} decision on {ticker}. This is a placeholder that would contain the actual reasoning in a real implementation."
        }
        
        # Generate mock signals from each selected analyst
        for analyst in selected_analysts:
            agent_name = f"{analyst}_agent"
            ...
            analyst_signals[agent_name][ticker] = {
                "signal": signal,
                "confidence": confidence,
                "reasoning": f"Mock {signal} reasoning from {analyst} for {ticker}"
            }
...
with st.sidebar:
    ...
    # Margin requirement
    margin_requirement = st.slider(
        "Margin Requirement (%)",
        min_value=0.0,
        max_value=100.0,
        value=0.0,
        step=5.0,
        help="Percentage of margin required for short positions"
    )
    ...
    selected_analysts_display = st.multiselect(
        ...
        default=["Ben Graham", "Bill Ackman", "Cathie Wood"]
    )
...
if 'portfolio' not in st.session_state:
    st.session_state.portfolio = {
        ...
        "margin_requirement": float(margin_requirement) / 100,  # Convert percentage to decimal
        "positions": {},
        "realized_gains": {}
    }
...
if not st.session_state.get('run_complete', False):
    ...
    # Show a note that this is a placeholder version
    st.warning("""
    **Note**: This is a demonstration version that generates random mock signals and trading decisions.
    To connect to the real AI Hedge Fund backend, you'll need to fix the import issues in your project.
    """)
